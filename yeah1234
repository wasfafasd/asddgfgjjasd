...
-- library functions 
	-- misc functions
		...
		function library:config_list_update()
		    ...
		    for _, file in next, listfiles(library.directory .. "/configs") do
		        ...
		        if name and name:sub(-4) == ".cfg" then
		            name = name:sub(1, -5)
		        end
    ...
    local auto_load_path = library.directory .. "/configs/auto_load.json"
    
    function library:auto_load_config()
        if not isfile(auto_load_path) then
            return library:notification({text = "No auto-load config found.", time = 3})
        end
    
        local success, config_json = pcall(readfile, auto_load_path)
        if success then
            library:load_config(config_json)
            library:notification({text = "Auto-loaded config.", time = 3})
        else
            library:notification({text = "Failed to load auto config.", time = 3})
        end
    end
    
    function library:set_auto_load_config(name)
        local path = library.directory .. "/configs/" .. name .. ".cfg"
        if not isfile(path) then
            return library:notification({text = "Config not found: " .. name, time = 3})
        end
    
        local success, config_json = pcall(readfile, path)
        if success then
            writefile(auto_load_path, config_json)
            library:notification({text = "Set auto-load config: " .. name, time = 3})
        else
            library:notification({text = "Failed to set auto-load config.", time = 3})
        end
    end
    
    function library:clear_auto_load_config()
        if isfile(auto_load_path) then
            delfile(auto_load_path)
            library:notification({text = "Cleared auto-load config.", time = 3})
        else
            library:notification({text = "No auto-load config to clear.", time = 3})
        end
    end
	...
	-- elements 
		...
		function library:window(properties)
			...
			-- main window
				local main_window = library:panel({
					name = properties and properties.name or " | ", 
			...
			-- theming 
				...
				local watermark = library:watermark({default = os.date(' |  - %b %d %Y - %H:%M:%S')})  

				task.spawn(function()
					while task.wait(1) do 
						watermark.change_text(os.date(' - Paid - %b %d %Y - %H:%M:%S'))
					end 
				end) 
			...
			-- cfg holder
				...
				local section = column:section({name = "Options"})
					...
					section:button({name = "Delete", callback = function()
					    local path = library.directory .. "/configs/" .. flags["config_name_list"] .. ".cfg"
					    if isfile(path) then
					        delfile(path)
					        library:config_list_update()
					    end
					...
					section:button_holder({})
		          section:button({name = "Auto Load Config", callback = function()
		              library:auto_load_config()
		          end})
		          
		          section:button({name = "Set Auto Load", callback = function()
		              library:set_auto_load_config(flags["config_name_list"])
		          end})
		          
		          section:button({name = "Clear Auto Load", callback = function()
		              library:clear_auto_load_config()
		          end})
					section:button_holder({})
		...
		function library:Notify(properties)
		    return self:notification(properties)
		end

		library.notifications = library.notifications or {}
		library.notif_holder = notif_holder
